...
class TestExtractRepoNameFromUrl:
    ...
    def test_extract_repo_name_github_standard_url(self):
        ...
        result = self.db_manager.extract_repo_name_from_url(github_url, "github")
        assert result == "owner_repo"
        
        # Test GitHub URL with .git suffix
        github_url_git = "https://github.com/owner/repo.git"
        result = self.db_manager.extract_repo_name_from_url(github_url_git, "github")
        assert result == "owner_repo"

        # Test GitHub URL with trailing slash
        github_url_slash = "https://github.com/owner/repo/"
        result = self.db_manager.extract_repo_name_from_url(github_url_slash, "github")
        assert result == "owner_repo"
    ...
    def test_extract_repo_name_gitlab_urls(self):
        ...
        result = self.db_manager.extract_repo_name_from_url(gitlab_url, "gitlab")
        assert result == "owner_repo"
        
        # Test GitLab URL with subgroups
        gitlab_subgroup = "https://gitlab.com/group/subgroup/repo"
        result = self.db_manager.extract_repo_name_from_url(gitlab_subgroup, "gitlab")
    ...
    def test_extract_repo_name_bitbucket_urls(self):
        ...
        result = self.db_manager.extract_repo_name_from_url(bitbucket_url, "bitbucket")
        assert result == "owner_repo"
    ...
    def test_extract_repo_name_local_paths(self):
        ...
        result = self.db_manager.extract_repo_name_from_url("/home/user/projects/my-repo", "local")
        assert result == "my-repo"

        result = self.db_manager.extract_repo_name_from_url("/var/repos/project.git", "local")
    ...
    def test_extract_repo_name_current_implementation_bug(self):
        ...
        try:
            ...
            result = self.db_manager.extract_repo_name_from_url("https://github.com/owner/repo")
        ...
        result = self.db_manager.extract_repo_name_from_url(local_path, "local")
        assert result == "my-repo"
        
        # Test local path with .git suffix
        local_git = "/var/repos/project.git"
        result = self.db_manager.extract_repo_name_from_url(local_git, "local")
        assert result == "project"
    ...
    def test_extract_repo_name_edge_cases(self):
        ...
        result = self.db_manager.extract_repo_name_from_url(short_url, "github")
        assert result == "repo"
        
        # Test single directory name
        single_name = "my-repo"
        result = self.db_manager.extract_repo_name_from_url(single_name, "local")
        assert result == "my-repo"
